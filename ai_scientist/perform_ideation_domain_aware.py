"""
Modified version of perform_ideation_temp_free.py that adapts the system prompt
to be domain-aware based on the workshop content.
"""
import argparse
import json
...
# Base system prompt template - will be customized based on workshop content
base_system_prompt = """You are an experienced researcher who aims to propose high-impact research ideas resembling exciting grant proposals. Feel free to propose any novel ideas or experiments; make sure they are novel. Be very creative and think out of the box. Each proposal should stem from a simple and elegant question, observation, or hypothesis about the topic. For example, they could involve very interesting and simple interventions or investigations that explore new possibilities or challenge existing assumptions. Clearly clarify how the proposal distinguishes from the existing literature.

Ensure that the proposal does not require resources beyond what an academic lab could afford. These proposals should lead to papers that are publishable at top conferences in the field.

IMPORTANT: Always use the SearchEntrez tool for literature searches. When using the search tool, make sure your search queries are directly related to the workshop topic. Your search queries should be specific and focused on the domain of the workshop.
...
ARGUMENTS:
<If ACTION is "SearchEntrez" or "SearchSemanticScholar", provide the search query as {{"query": "your search query"}}. If ACTION is "FinalizeIdea", provide the idea details as {{"idea": {{ ... }}}} with the IDEA JSON specified below.>
...
Begin by generating an interestingly new high-level research proposal that differs from what you have previously proposed. Make sure your proposal is directly related to the workshop topic.
"""
...
def extract_domain_keywords(workshop_description: str) -> List[str]:
    """
    Extract domain-specific keywords from the workshop description.
    This is a simple implementation that extracts keywords from the title and keywords section.
    """
    keywords = []

    # Extract title
    title_match = re.search(
        r"# Title: (.*?)$", workshop_description, re.MULTILINE)
    if title_match:
    ...
    keywords_match = re.search(
        r"## Keywords\s+(.*?)$", workshop_description, re.MULTILINE)
    if keywords_match:
...
def customize_system_prompt(workshop_description: str) -> str:
    """
    Customize the system prompt based on the workshop description.
    """
    domain_keywords = extract_domain_keywords(workshop_description)

    # Create a domain-specific addition to the system prompt
    domain_specific_text = ""
    if domain_keywords:
        domain_specific_text = f"""
IMPORTANT: This workshop is about {', '.join(domain_keywords)}.
Always use the SearchEntrez tool for literature searches.
When using the search tool, make sure your search queries are directly related to these topics.
Your search queries should include relevant terms from the workshop domain.
"""

    # Insert the domain-specific text into the base system prompt
    customized_prompt = base_system_prompt.replace(
        "IMPORTANT: Always use the SearchEntrez tool for literature searches. When using the search tool,",
        domain_specific_text +
        "\nIMPORTANT: Always use the SearchEntrez tool for literature searches. When using the search tool,"
    )
...
def generate_domain_aware_idea(
    idea_fname: str,
    client: Any,
    model: str,
    ...
    # Customize the system prompt based on the workshop description
    system_prompt = customize_system_prompt(workshop_description)
    print(f"Using domain-aware system prompt:\n{system_prompt}")
                ...
                try:
                    ...
                    if action in tools_dict:
                        ...
                        try:
                            ...
                            raise ValueError(
                                f"Invalid arguments JSON for {action}.")
                        ...
                        try:
                            ...
                            raise ValueError(
                                "Invalid arguments JSON for FinalizeIdea.")
                    else:
                        print(
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generate domain-aware research ideas"
    )
    parser.add_argument(
        "--model",
        type=str,
    ...
    parser.add_argument(
        "--workshop-file",
        type=str,
        default="ai_scientist/ideas/multitask_antibody_maturation.md",
    ...
    # Always default to Entrez API unless explicitly specified
    ...
    # Ensure the first tool is always the Entrez tool by default
    if tools[0].name != "SearchEntrez" and not args.use_semantic_scholar:
        print("Forcing use of Entrez API as default")
        tools[0] = entrez_tool

    # Update tools dictionary
    tools_dict = {
        tool.name: tool for tool in tools if isinstance(tool, BaseTool)}
    ...
    print(
        f"Using workshop description from {args.workshop_file} for idea generation.")
    ...
    ideas = generate_domain_aware_idea(
